(function initVocabularyNotebookCore(root, factory) {
  ...
  function makeCardId(card = {}) {
    const material = [
      ...
      normalizeText(card.explanationLanguage || ""),
    ].join("\u241f");
  ...
  function buildCsv(cards = [], sources = []) {
    ...
    const rows = (Array.isArray(cards) ? cards : []).map((card) => {
      ...
      return {
        ...
        explanationLanguage: card.explanationLanguage || source.explanationLanguage || "",
  ...
  function buildAnkiCsv(cards = [], sources = []) {
    ...
    for (const card of Array.isArray(cards) ? cards : []) {
      ...
      const tags = [
        ...
        card.type,
        source.sourceLanguage || card.sourceLanguage,
        explanationLanguage ? `explanation-${explanationLanguage}` : "",
      ].filter(Boolean).join(" ");
