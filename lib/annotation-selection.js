...
function contextualLevelSelectionRules() {
  return [
    "Use these learner bands: beginner = A1-A2, intermediate = B1-B2, advanced = C1-C2 and above.",
    "For every possible ordinary annotation, first judge the contextual expression or sense as one learning unit and assign the lowest learner band at which annotating that contextual meaning would provide meaningful learning value.",
    "Assign level to the contextual expression or sense, not to the difficulty of its component words. Basic words can form intermediate or advanced idioms, phrasal verbs, metaphorical or context-sensitive senses, reusable constructions, and technical terms.",
    "Conversely, high frequency, broad reusability, or being a familiar discourse marker does not make an expression suitable for a higher learner band when its contextual meaning is already comfortably learned at a lower band.",
    "For the selected target level, omit candidates whose lowest useful learner band is lower than the selected band.",
    "Do not downgrade a recognized domain term merely because its component words are simple; judge the learning value of the term or concept as a unit in context.",
  ];
}
...
function buildCoverageCompletionPrompt({
  ...
  return [
    ...
    "Do not fill a quota, annotate elementary material, or repeat an existing target. Overlapping spans are allowed when they explain genuinely different teachable units.",
    ...
    `Write meaningJa in natural ${explanationLanguage} as a short gloss. noteJa is optional: use it only when it does a different job from the gloss, such as explaining usage, contextual interpretation, register, collocation, constructional behavior, or a technical definition. If it would merely paraphrase meaningJa or say that the item is an expression/phrase, return an empty string. In Japanese, use plain style rather than desu/masu style.`,
    "Schema:",
    '{"annotations":[{"text":"exact reviewText substring","type":"word|collocation|formula|construction|idiom|term","meaningJa":"short gloss","noteJa":"useful extra explanation or empty string","example":"short source-language example","pattern":"generalized construction or empty","coreRanges":[{"start":0,"end":8}],"start":0,"end":10,"priority":5,"reliability":"high"}]}',
  ].join("\n");
}
...
function mergeUniqueNonOverlappingAnnotations(existing, additions) {
  ...
  for (const item of rankEligibleAnnotations([...(existing || []), ...(additions || [])])) {
    ...
    const key = `${item.start}:${item.end}:${text.trim().toLocaleLowerCase()}:${String(item.type || "")}`;
