(function initReasonSelection(root, factory) {
  ...
  function minSelectedRank(levels) {
    return Math.min(...normalizeSelectedLevels(levels).map((level) => LEVEL_RANK[level]));
  }
  ...
  function reasonProfile(candidate, levels) {
    ...
    if (!meta) return { tags: [], codes: [], strong: false, bestValue: "low" };

    const tags = [];
    const codes = [];
    const learnerFloor = minSelectedRank(selected);
    ...
    const bestValue = bestAnnotationValue(meta, selected);
    ...
    return { tags: uniqueTags, codes: uniqueCodes, strong, bestValue };
  }
  ...
  function judgedEligible(candidate, density, levels) {
    ...
    if (isAdvancedRecallCandidate(candidate, density, levels)) return true;
    if (Number(density) >= 3) {
