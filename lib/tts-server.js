...
const TTS_MODES = new Set(["clear", "natural", "casual"]);
const TTS_MODE_INSTRUCTIONS = Object.freeze({
  ...
  natural: [
    ...
    "Use natural weak forms, linking, assimilation, and reductions where they normally occur.",
  ...
  casual: [
    ...
    "Use natural weak forms, linking, assimilation, flapping, contractions, and occasional elision where appropriate.",
    "Keep content words prominent and let function words reduce naturally; do not merely read the text faster.",
  ].join(" "),
});
...
function buildSpeechInstructions({ mode, customInstructions } = {}) {
  ...
  return [
    ...
    TTS_MODE_INSTRUCTIONS[normalizedMode],
    custom ? `Additional delivery instructions from the user: ${custom}` : "",
...
function buildSpeechPayload({ text, voice, model, speed, mode, customInstructions } = {}) {
  return {
    model: String(model || DEFAULT_TTS_MODEL),
    input: String(text || ""),
    voice: normalizeVoice(voice),
    instructions: buildSpeechInstructions({ mode, customInstructions }),
    response_format: "mp3",
