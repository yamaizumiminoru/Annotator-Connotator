...
const CONTEXT_TOOL = {
  ...
  name: "get_context",
  description: [
    "Read source text immediately before and after the passage the user selected.",
    "Call this when the answer materially depends on local discourse context, reference, ellipsis, tense/aspect,",
    "stance, implicature, irony, register, ambiguity, syntax interacting with nearby text, or author intent.",
    "Request the smallest useful window first. You may call again with a larger window if necessary.",
  ].join(" "),
  parameters: {
    type: "object",
    properties: {
...
function buildQuestionInstructions(explanationLanguage = "en") {
  ...
  return [
    ...
    "You have NOT been given the surrounding source text unless you call get_context.",
    "Never imply that you have read surrounding context that you have not requested.",
    "If nearby wording is materially relevant to the answer, call get_context before answering.",
    ...
    "If the question is reliably answerable from the selected passage plus general language knowledge, answer directly without calling the tool.",
...
function buildInitialQuestionInput({ selectedText, question } = {}) {
  return [
    "Selected passage:",
    "<<<SELECTED>>>",
    String(selectedText || ""),
...
async function answerQuestion(payload, options = {}) {
  ...
  let response = await callResponses(responseRequestBody({
    model,
    instructions,
    input: buildInitialQuestionInput({ selectedText, question }),
  }), { ...options, apiKey });
  responses.push(response);

  let contextRequests = 0;
  let contextCharsSent = 0;
