...
function repairAnnotationOffsets(sourceText, annotations) {
  ...
  return annotations.map((item) => {
    ...
    const located = locateExactSpan(sourceText, annotationText, item.start, item.end);
    // Overlapping spans can teach different targets. Judge them independently;
    // exact duplicates are handled by the later merge, not by offset repair.
    return located ? { ...item, start: located.start, end: located.end } : null;
  }).filter(Boolean);
}
