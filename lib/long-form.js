...
const { normalizeFormattingSpans } = require("./source-formatting");
...
function mergeChunkResults(sourceText, analyzedChunks, fallback = {}) {
  ...
  for (const entry of analyzedChunks) {
    ...
    shiftedFormattingSpans.push(...(Array.isArray(result.formattingSpans) ? result.formattingSpans : []).map((item) => ({
      ...item,
      start: Number(item.start) + offset,
      end: Number(item.end) + offset,
    })));
  ...
  const formattingSpans = normalizeFormattingSpans(source, shiftedFormattingSpans);
